...
class TestDict:
    ...
    def test_dict_duplicate_loads(self):
        assert xorjson.loads(b'{"1":true,"1":false}') == {"1": False}
    ...
    def test_dict_unicode(self):
        ...
        assert xorjson.loads(ref) == obj
        assert xorjson.loads(ref)["🐈"] == "value"
    ...
    def test_dict_invalid_key_dumps(self):
        ...
        with pytest.raises(xorjson.JSONEncodeError):
            xorjson.dumps({1: "value"})
        with pytest.raises(xorjson.JSONEncodeError):
            xorjson.dumps({b"key": "value"})
    ...
    def test_dict_invalid_key_loads(self):
        ...
        with pytest.raises(xorjson.JSONDecodeError):
            xorjson.loads('{1:"value"}')
        with pytest.raises(xorjson.JSONDecodeError):
            xorjson.loads('{{"a":true}:true}')
    ...
    def test_dict_pop_replace_first(self):
        ...
        data.pop("id")
        assert xorjson.dumps(data) == b'{"other":"any"}'
        data["id"] = "new"
        assert xorjson.dumps(data) == b'{"other":"any","id":"new"}'
    ...
    def test_dict_pop(self):
        ...
        data = {"id": "any"}
        data.pop("id")
        assert xorjson.dumps(data) == b"{}"
        data["id"] = "new"
        assert xorjson.dumps(data) == b'{"id":"new"}'

    def test_in_place(self):
        "Mutate dict in-place"
        data = {"id": "any", "static": "msg"}
        data["id"] = "new"
        assert xorjson.dumps(data) == b'{"id":"new","static":"msg"}'
    ...
    def test_dict_0xff(self):
        ...
        data["112"] = 1
        data["113"] = 2
        assert xorjson.loads(xorjson.dumps(data)) == data
    ...
    def test_dict_dict(self):
        ...
        assert xorjson.dumps(C().__dict__) == b'{"a":0,"b":1}'
